...
modifier accountAlreadyExist(uint256 _acNumber) {
		...
		require(check== false, "Account already exists,please Change your Account Number");
		_;
	}
	...
	event eventAccountCreation(address accountAddress, uint accountNumber);
...
/*----------------------------------------functions -------------------------------------------------*/
    ...
    function deposit(uint256 _accountNumber, uint256 _depositBalance) public  {
        mappedbankaccount[msg.sender][_accountNumber].balance += _depositBalance;
    ...
    function withdraw(uint256 _accountNumber, uint256 _withdrawBalance) public {
        mappedbankaccount[msg.sender][_accountNumber].balance -= _withdrawBalance;
    ...
    function getDataOfUser() public view returns (userdata[] memory) {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            if (
                mappedbankaccount[msg.sender][accounts[i]].exists == true
            ) {
        ...
        returns (bankdata[] memory)
    {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            for (uint256 j = 0; j < accounts.length; j++) {
                if (
                    mappedbankaccount[addresses[i]][accounts[j]]
                        .bank_name == _name
                ) {
                    // new struct to store data
    ...
    function tranferAmount(
        ...
        require(
            mappedbankaccount[msg.sender][_fromAccountNo].balance >= _transferBalance,
            "insufficient balance"
        );
        mappedbankaccount[_toAddress][_toAccountNo].balance += _transferBalance;
        mappedbankaccount[msg.sender][_fromAccountNo].balance -= _transferBalance;
        emit eventBalanceTransfer(_fromAccountNo, _toAccountNo, _transferBalance);
    }
    ...
    function transferOwner(address _newAdminaddr) public {
        require(msg.sender == owner,"Permission denied as you are not owner of this Account");
        owner =_newAdminaddr;
